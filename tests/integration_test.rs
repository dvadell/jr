...
fn test_min_max_value_angelweb_output() {
    ...
    let json_payload: Value =
        serde_json::from_str(&file_content).expect("Failed to parse JSON from file content");
...
fn test_graph_type_angelweb_output() {
    ...
    assert_eq!(
        json_payload["short_name"].as_str(),
        Some("test_graph_type_metric")
    );
}
